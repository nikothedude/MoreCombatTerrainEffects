...
class blackHoleEffectScript(
    ...
    private fun applyGravity(amount: Float) {
        ...
        for (entry in pluginsToIntensity.entries) {
            ...
            for (entity: CombatEntityAPI in engine.getAllObjects()) {
                ...
                val modifiedIntensity = getIntensityForEntity(entity, intensity)
                val angle = pluginsToAngle[plugin]!!
                entity.applyForce(angle, modifiedIntensity)
            }
        }
    }

    private fun getIntensityForEntity(entity: CombatEntityAPI, baseIntensity: Float): Float {
        var coronaResist = 1f
        var timeMult = 1f
        ...
        if (entity is ShipAPI) {
            ...
            coronaResist = entity.mutableStats.dynamic.getStat(Stats.CORONA_EFFECT_MULT).modifiedValue
        }
        val adjustedIntensity = baseIntensity * BLACKHOLE_BASE_GRAVITY
        val totalMult = timeMult + engineMult-1
        return ((adjustedIntensity)/coronaResist)*totalMult
    }
    ...
    private fun getTimeMultForShip(ship: ShipAPI): Float {
        val coronaResist = ship.mutableStats.dynamic.getStat(Stats.CORONA_EFFECT_MULT).modifiedValue
        return timeMult/coronaResist
    }
    ...
    override fun handleNotification(amount: Float) {
        ...
        engine.maintainStatusForPlayerShip(
            ...
            icon,
            "Black Hole",
            "Time dilation increased by ${getTimeMultForShip(engine.playerShip)}%",
            true)
        engine.maintainStatusForPlayerShip(
